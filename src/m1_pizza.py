...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  4 pink dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 100), 35)
    draw_points_on_circle(window, circle, 4, 'pink')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  140 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'yellow'
    circle.outline_thickness = 1
    pizza(window, circle, 140, 'black', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(points)):
        peice = rg.Line(points[k], center)
        peice.color = color
        peice.attach_to(window)
        peice.thickness = thickness
    window.render()
    """
    What comes in:
      -- an rg.RoseWindow
...
def run_test_polygon():
    ...
    # Tests 3:
    title = ('POLYGON tests 3:  4 segments with thick black lines;')
    window = rg.RoseWindow(400, 400, title)
